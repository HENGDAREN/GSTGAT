...
parser.add_argument('--gat_true', type=bool, default=True, help='use or not GAT, Ture: use')
# 文件路径e
...
parser.add_argument('--earlystop', type=int, default=50, help='number for early stopping')
...
# 模型参数e
parser.add_argument('--embed_dim', type=int, default=10, help='dim of node')
...
def main(args):
    ...
    # todo  --------------这个可以说是我见过最有用的提升速度的一个trick了，真的快了不少---------
    ...
    # 生成数据读取器
    train_loader, vaild_loader, test_loader = data.DataLoader(train_dataset, args.batch_size,
                                                              shuffle=args.input_shuffle), \
                                              data.DataLoader(vaild_dataset, args.batch_size), \
                                              data.DataLoader(test_dataset, args.batch_size)

    # 开始训练
    earlystopping = EarlyStopping(patience=args.earlystop, verbose=2, delta=0)
    ...
    for epoch in range(args.epoch):
        loss_one_epoch = []
        loss_one_valid_epoch = []

        init_start = time.time()
        count = 0
        accumulation_steps = 4
        for iter, (data_x, data_y, train_te) in enumerate(train_loader):
            # print(data_x, '-----------------------------------' , data_y)
            model.train()
            count += 1
            # 这个到底用不用放到variable中去,  train_te 不需要扰乱， shape = batch_size, 16, 2
            train_x, train_y, train_TE = Variable(data_x), Variable(data_y), Variable(train_te)
            train_x, train_y, train_TE = train_x.to(device), train_y.to(device), train_TE.to(device)

            # 这里id其实是None，但是传进去却走的不是None这条路，这里奇特的就是传进去的是None，但是判定却不是None，
            pred_y, gl_loss = model(train_x, train_TE)
            # todo 这里需要反归一化
            pred_y = pred_y * std + mean
            train_y = train_y * std + mean
            if args.gat_true:
                loss = metric(pred_y, train_y) + args.order * torch.mean(gl_loss)
                #loss = metric(pred_y, train_y)
            else:
                loss = metric(pred_y, train_y)
            ...
            loss.backward()
            # 梯度裁剪
            torch.nn.utils.clip_grad_norm_(model.parameters(), weight_value)
            torch.nn.utils.clip_grad_value_(model.parameters(), weight_value)
            # 这里是对模型参数进行优化
        ...
        # 验证集
        if epoch % 1 == 0 or epoch > int(args.epoch * 0.7):
            with torch.no_grad():
                model.eval()
                for valid_x, valid_y, val_TE in vaild_loader:
                    valid_x, valid_y, val_TE = valid_x.to(device), valid_y.to(device), val_TE.to(device)
                    valid_pred_y, _ = model(valid_x, val_TE)
                    # todo 反归一化
                    valid_pred_y = valid_pred_y * std + mean
                    valid_y = valid_y * std + mean
                    valid_loss = metric(valid_pred_y, valid_y)
                    loss_one_valid_epoch.append(valid_loss.item())

            earlystopping.save_single(loss_one_valid_epoch[-1], model, model_save_path)
            if earlystopping.early_stop:
                print('-----------Early Stop -----------------')
                break

        scheduler.step(loss_one_valid_epoch[-1])
        one_epoch_time = time.time() - init_start
        print('epoch {} is finished. the one epoch train set loss is {}, the one valid loss is {}'.format(
            epoch, loss_one_epoch[-1], loss_one_valid_epoch[-1]
        ))
        print('----------running one epoch time is : {}------------'.format(one_epoch_time))
        loss_list.append(loss_one_epoch[-1])
        valid_loss_list.append(loss_one_valid_epoch[-1])

    # 保存训练损失
    loss_save_path = args.model_save_path + 'loss.txt'
    vaild_save_path = args.model_save_path + 'vaild_loss.txt'
    np.savetxt(str(loss_save_path), np.array(loss_list))
    np.savetxt(str(vaild_save_path), np.array(valid_loss_list))
    # 测试
...
if __name__ == '__main__':
    ...
    epoch = 300
    window_list = [35]
    pre_lenList = [1,3,7]  # 1, 3, 7, 15
    batch_size = 128  # 128 8
    learning_rateList = [0.001]  # , 0.003, 0.0001, 0.0003   #0.001
    # 模型参数
    embed_dim_list = [10]  # 10,20 40, 10,     #10
    channelsList = [32]  # , 32 , 8     #32
    dropoutlist = [0.3]  # , 0.2     #0.3
    gammalist = [0.01]  # , 0.01, 0.0001  0.1, 1que
    orderlist = [0.01]  # , 0.1, 0.01, 0.0001  1,
    dilation = 2
    TCN_layers = 3
    sst_file = 'data/Bo_Hai.csv'  # Nan_Hai_samePaper.csv
    step_size = 20
    file = 'SST'
    gatnheads = 4  # only control GAT heads
    gdep_list = 2  # depth of mix-gat module
    gat_featureList = 10  # the out_feature of gatmodule
                            ...
                            for embed in embed_dim_list:
                                for learning_rate in learning_rateList:
                                    for weight_decay in weight_decaylist:
                                        log_file = 'D:/gzh/GAT-sst/log/log10.5_bohai_noloss/{}_{}_{}_{}_{}_{}_{}_{}_log'. \
                                            format(file, i, p, channels, dropout, gamma, order, embed)
                                        model_save_path = 'D:/gzh/GAT-sst/model/model10.5_bohai_noloss/{}_{}_{}_{}_{}_{}_{}_{}'. \
                                            format(file, i, p, channels, dropout, gamma, order, embed)
